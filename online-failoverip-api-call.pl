...
#my $online_api_url = "https://www.raphux.com/api/v1/server/failover";
...
sub print_usage(){
	print("Usage : $0 --action=ACTION --apitoken=API_TOKEN --failoverip=FAILOVER_IP --destinationip=DESTINATION_IP\n");
	print("Where :\n");
...
if ( "$action" eq "status" ){
	...
	$client->GET($url);
	my $json_response = decode_json($client->responseContent());
	foreach my $ip_failover (@$json_response) {
		if ( $ip_failover->{'source'} eq "$failoverip" ) {
			if (( ! $ip_failover->{'destination'}) and (! $destinationip)){
				...
				exit 0;
			} elsif (( $destinationip ) and ( $ip_failover->{'destination'} ne "$destinationip" )){
				print "Critical : Failover IP is NOT assigned to $destinationip !\n";
				exit 1;
			}
		}
